...
class TrashedCategoryNewsController extends Controller
{
    public function index(){
        $filters = request()->only(['per_page', 'sortorder', 'keyword']);
        $categories = CategoryNews::onlyTrashed()->search($filters['keyword'] ?? null)
    ...
    public function forceDelete($id)
    {
        if (!empty($id) && is_numeric($id)) {
            try {
                $category = CategoryNews::onlyTrashed()->find($id);
                ...
                if ($category) {
                    ...
                    return response()->json(['message' => 'Danh mục tin không tồn tại', 'status' => 'error'], 404);
                }
            } catch (QueryException $e) {
                if ($e->getCode() == '23000') {
                    return response()->json(['message' => 'Không thể xóa danh mục tin vì có dữ liệu liên quan', 'status' => 'error'], 400);
                }
